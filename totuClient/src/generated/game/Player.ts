...
export const Player = {
  encode(message: Player, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    for (const v of message.hand) {
      Card.encode(v!, writer.uint32(50).fork()).ldelim();
    }
    for (const v of message.play) {
      Card.encode(v!, writer.uint32(58).fork()).ldelim();
    }
    for (const v of message.discard) {
      Card.encode(v!, writer.uint32(66).fork()).ldelim();
    }
    for (const v of message.innerCircle) {
      Card.encode(v!, writer.uint32(74).fork()).ldelim();
    }
    if (message.tokens !== 0) {
